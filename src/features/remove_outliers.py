...
dataset, outliers, X_scores = mark_outliers_lof(df[df['label'] == label], outlier_columns, n=20)
for col in outlier_columns:
    plot_binary_outliers(dataset=dataset, col=col, outlier_col='outlier_lof', reset_index=True)
...
for col in outlier_columns:
    for label in df['label'].unique():
        ...
        # Probably an easier way to do this without creating so many copies of the DFs.
        n_outliers = len(dataset) - len(dataset[col].dropna())
        print(f'Removed {n_outliers} from {col} for {label}')
# --------------------------------------------------------------
# Export new dataframe
# --------------------------------------------------------------
path = '../../data/interim/02_outliers_removed_chauvenet.parquet'
outliers_remove_df.to_parquet(path)